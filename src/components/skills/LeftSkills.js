import React from 'react'

const LeftSkills = () => {
  return (
    <div className="w-full lgl:w-1/2 flex flex-col gap-2  ">
    
      <h4 className=" text-lg font-bold items-center flex flex-col mb-12">Technical Skills</h4>
     
      <div className='flex flex-row justify-between w-full ' >
      <div className=" text-lg font-bold text-designColor ">HTML</div>
      <div className=" text-lg font-bold text-designColor">90%</div>
      
      
      
      </div>
      <div className='w-full h-3 rounded-3xl bg-black'> 
      <div
      className="h-full rounded-3xl bg-designColor w-11/12 -"
      
    ></div> </div>
    <div className='flex flex-row justify-between w-full mt-5' >
      <div className=" text-lg font-bold text-designColor ">CSS</div>
      <div className=" text-lg font-bold text-designColor">90%</div>
      
      
      
      </div>
      <div className='w-full h-3 rounded-3xl bg-black'> 
      <div
      className="h-full rounded-3xl bg-designColor w-11/12 - "
      
    ></div> </div>
    <div className='flex flex-row justify-between w-full mt-5' >
      <div className=" text-lg font-bold text-designColor ">JS</div>
      <div className=" text-lg font-bold text-designColor">80%</div>
      
      
      
      </div>
      <div className='w-full h-3 rounded-3xl bg-black'> 
      <div
      className="h-full rounded-3xl bg-designColor w-4/5 - "
      
    ></div> </div>
    <div className='flex flex-row justify-between w-full mt-5' >
      <div className=" text-lg font-bold text-designColor ">REACT JS</div>
      <div className=" text-lg font-bold text-designColor">85%</div>
      
      
      
      </div>
      <div className='w-full h-3 rounded-3xl bg-black'> 
      <div
      className="h-full rounded-3xl bg-designColor w-5/6 - "
      
    ></div> </div>
    <div className='flex flex-row justify-between w-full mt-5' >
      <div className=" text-lg font-bold text-designColor ">EXPRESS JS</div>
      <div className=" text-lg font-bold text-designColor">80%</div>
      
      
      
      </div>
      <div className='w-full h-3 rounded-3xl bg-black'> 
      <div
      className="h-full rounded-3xl bg-designColor w-4/5 - "
      
    ></div> </div>
    <div className='flex flex-row justify-between w-full mt-5' >
      <div className=" text-lg font-bold text-designColor ">DART</div>
      <div className=" text-lg font-bold text-designColor">80%</div>
      
      
      
      </div>
      <div className='w-full h-3 rounded-3xl bg-black'> 
      <div
      className="h-full rounded-3xl bg-designColor w-4/5 - "
      
    ></div> </div>
    <div className='flex flex-row justify-between w-full mt-5' >
      <div className=" text-lg font-bold text-designColor ">FLUTTER</div>
      <div className=" text-lg font-bold text-designColor">80%</div>
      
      
      
      </div>
      <div className='w-full h-3 rounded-3xl bg-black'> 
      <div
      className="h-full rounded-3xl bg-designColor w-4/5 - "
      
    ></div> </div>

      
   
    </div>

  )
}

export default LeftSkills
